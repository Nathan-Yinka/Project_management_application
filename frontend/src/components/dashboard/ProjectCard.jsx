...
const ProjectCard = ({ project, onTaskClick }) => {
  ...
  const changeStatus = async (status) => {
    ...
    // console.log(status)
    changeTaskStatus(project.id, status);
  ...
  return (
    <div className="mx-2 relative" onClick={() => onTaskClick(project)}>
      <div className="bg-white rounded-lg p-4 w-[15rem] min-h-[15rem] md:w-[18rem] md:h-[17rem] border border-gray-200 relative">
        ...
        <div className="flex items-center mb-5">
          <span
            className={`${avatarColor} capitalize rounded-full h-8 w-8 flex items-center justify-center text-white mr-2`}
          >
            {project?.user?.name.charAt(0)}
          </span>
          <span className="text-gray-700 font-medium capitalize text-sm md:text:md">
            {project?.user?.name}
          </span>
        </div>
        ...
        <h3 className="md:text-xl font-semibold mb-4 line-clamp-2">
          {project.name}
        </h3>
        <p className="text-gray-600 line-clamp-2 text-sm md:text:md">
          {project.description}
        </p>
      </div>
    </div>
