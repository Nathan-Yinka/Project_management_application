...
const ProjectCard = ({ project, onTaskClick }) => {
  ...
  return (
    <div className="mx-2 relative" onClick={() => onTaskClick(project)}>
      <div className="bg-white rounded-lg p-4 max-w-[18rem] border border-gray-200 relative md:h-[17rem]">
          ...
          >
            ...
            {isModalOpen && (
              ...
              >
                ...
                {["Done", "In Progress", "Complete", "Abandoned"].map((status) => (
                  <button
                    key={status}
        ...
        <div className="flex items-center mb-5">
          ...
          <span className="text-gray-700 font-medium text-sm md:text:md">{project.name}</span>
        </div>
        <div className="flex items-center text-gray-400 text-sm md:text-md mb-4">
          <img src={clock} alt="Clock icon" />
          <span className="ml-2">{project.date}</span>
        </div>
        <h3 className="md:text-xl font-semibold mb-4 line-clamp-2">{project.title}</h3>
